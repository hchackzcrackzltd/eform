...
<div class="row">
  <div class="col-xs-6">
    @if ($data->getFile->where('type', 2)->first())
      <img src="data:{{Storage::mimeType('attach/'.$data->getFile->where('type', 2)->first()->temp)}};base64,{{base64_encode(Storage::get('attach/'.$data->getFile->where('type', 2)->first()->temp))}}" alt="Picture Profile" class="img-circle col-xs-5"><br><br>
    @else
...
<p><i class="label label-success">IQ:</i> {{(empty($data->iq))?'ไม่ระบุ':$data->iq}}</p>
<p><i class="label label-primary">EQ:</i> {{(empty($data->eq))?'ไม่ระบุ':$data->eq}}</p>
<p><i class="label label-warning">MBTI:</i> {{(empty($data->mbti))?'ไม่ระบุ':$data->mbti}}</p>
  </div>
  ...
  <div class="col-xs-6">
    ...
    <div class="col-xs-12 clearfix">
      ...
      <hr>
        ...
        @foreach ($data->getChildens as $key=>$value)
          <div class="col-xs-12">
          ...
          <div class="col-xs-12">
            @foreach ($data->getEdu as $key=>$value)
              <div class="col-xs-12">
                ...
                <p>{{$master_edu->where('id',$value->edu_id)->first()->name}}</p>
              </div>
              <div class="col-xs-12">
        ...
        <div class="col-xs-12 clearfix">
          ...
          <hr>
            ...
            @foreach ($data->getLang->groupBy('lang_id') as $key => $value)
              <div class="col-xs-12 col-md-6">
                <b>ภาษา {{$form_lang->where('id',$value[0]->lang_id)->first()->name}}</b>
              </div>
              <div class="col-xs-12 col-md-6">
            ...
            <hr>
              <div class="col-xs-12">
                @foreach ($data->getPositions as $value)
                  <p><b>ตำแหน่งที่ {{$loop->index+1}} :</b> {{$form_posit->where('id',$value->posit_id)->first()->name}}</p>
                @endforeach
...
<hr>
<div class="col-xs-12">
  <b class="h5">ท่านมีใบขับขี่หรือไม่ ? (Do you hold a driving licence?):</b>
</div>
<div class="col-xs-12 col-md-6">
  <b>รถยนต์ (Car)</b>
...
@if ($data->intv_format_when)
  <div class="col-xs-12">
    <b>ถ้าเคย เมื่อไร ? (If yes, Please explain ?)</b>
  ...
  <div class="col-xs-12"><br><br>
    ...
    <div class="table-responsive">
      <table class="table table-bordered text-center">
        ...
        <tbody>
          @foreach ($data->getFile->where('type',1) as $value)
            <tr>
              <td>{{$loop->index+1}}</td>
              <td><a href="{{route('candidate_viewfile',['fnm'=>$value->temp])}}" target="_blank">{{$value->name}}</a></td>
            </tr>
          @endforeach
        </tbody>
      </table>
