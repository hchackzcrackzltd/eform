...
@section('content')
  <div class="row">
    ...
    @if(!is_null(session('success')))
      ...
      <button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>
      <strong><i class="ace-icon fa fa-check"></i>&nbsp;&nbsp;สำเร็จ</strong>&nbsp;&nbsp;{{session('success')}}<br>
      </div>
    @endif
  </div>
  <div class="row" style=" background-color: rgba(231, 231, 231, 0.4);">
    <div id="modal-wizard-container" >
      ...
      <form id="efm" name="efm" action="{{route('eform.update',['eform'=>$data->id])}}" method="post" enctype="multipart/form-data" data-parsley-validate>
        {{csrf_field()}}
        {{method_field('PATCH')}}
        </form>
      <div class="modal-body step-content" style="border: 1px rgb(228, 224, 224) solid; background-color: rgba(255, 255, 255, 0.33);">
        <div class="step-pane active" data-step="1">
            @include('form_edit.section1')
        </div>

        <div class="step-pane" data-step="2">
            @include('form_edit.section2')
        </div>

        <div class="step-pane" data-step="3">
          @include('form_edit.section3')
        </div>

        <div class="step-pane" data-step="4">
          @include('form_edit.section4')
        </div>
      </div>
    </div>
